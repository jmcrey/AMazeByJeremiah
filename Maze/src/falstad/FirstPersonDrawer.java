...
public class FirstPersonDrawer extends DefaultViewer {
	...
	public FirstPersonDrawer(int width, int height, int map_unit, int step_size, Cells mazecells, Cells seencells, int map_scale, int[][] mazedists, int mazew, int mazeh, BSPNode bsp_root, Maze maze){
		...
		this.gw = new GraphicsWrapper(maze);
	...
	public void redraw(Maze maze, int state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		...
		gw.getGraphics().fillRect(0, 0, view_width, view_height/2);
		// draw dark gray background on upper half
		gw.setGraphicsColor("darkGray");
		gw.getGraphics().fillRect(0, view_height/2, view_width, view_height/2);
		...
		//gw.redrawPerson(maze.panel.getBufferGraphics(), state, px, py, view_dx, view_dy, walk_step, view_offset, rset, ang);
	...
	private void drawSegment(Seg seg, int ox1, int y1, int ox2, int y2) {
		...
		int xd = x2-x1;
		//gc.setColor(seg.col);
		gw.getGraphics().setColor(seg.getGraphics().getColor());
		boolean drawn = false;
		...
		while (x1i <= x2) {
			...
			gw.setPoint(x1i, x2);
			if (!rset.intersect(gw))
				break;
			x1i = gw.getPoint().x;
			int x2i = gw.getPoint().y;
			...
			gw.getGraphics().fillPolygon(xps, yps, 4);
			drawn = true;
			rset.remove(x1i, x2i);
			x1i = x2i+1;
