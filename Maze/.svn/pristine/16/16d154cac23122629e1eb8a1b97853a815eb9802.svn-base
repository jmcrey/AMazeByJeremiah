...
public class MapDrawer extends DefaultViewer {
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				if ((seencells.hasWallOnTop(x, y) || showMaze) && w)
					gw.getGraphics().drawLine(nx1, ny1, nx2, ny1);
				...
				if ((seencells.hasWallOnLeft(x, y) || showMaze) && w)
					gw.getGraphics().drawLine(nx1, ny1, nx1, ny2);
			}
		if (showSolution) {
	...
	public void draw_currentlocation(int view_dx, int view_dy) {
		gw.setGraphics(maze.panel.getBufferGraphics());
		//gw.getGraphics().setColor(Color.red);
		...
		gw.getGraphics().fillOval(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz);
		...
		gw.getGraphics().drawLine(ctrx, ctry, aptx, apty);
		gw.getGraphics().drawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
		gw.getGraphics().drawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy);
	}
	...
	public void draw_solution(int offx, int offy, int px, int py) {
		...
		while (d > 1) {
			...
			// do the graphics
			gw.getGraphics().drawLine(nx1, ny1, nx1+ndx, ny1+ndy);
