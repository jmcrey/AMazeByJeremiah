...
import edu.wm.cs.cs301.jeremiahmcreynolds.amazebyjeremiahmcreynolds.ui.GeneratingActivity;
...
public class Maze extends Application {
	...
	public boolean showMaze = false;		 	// toggle switch to show overall maze on screen
	public boolean showSolution = false;		// toggle switch to show solution in overall maze on screen
	public boolean solving = false;			// toggle switch 
	public boolean mapMode = false; // true: display map of maze, false: do not display map of maze
...
//		notifyViewerRedraw() ;
		...
		switch(method){
		case 2 : mazebuilder = new MazeBuilderEller(true, gw); // generate with Eller's algorithm
		break ;
		case 1 : mazebuilder = new MazeBuilderPrim(true, gw); // generate with Prim's algorithm
		break ;
		case 0: // generate with Falstad's original algorithm (0 and default), note the missing break statement
		default : mazebuilder = new MazeBuilder(true, gw); 
		break ;
		}
...
//		rootnode = root;
		addView(new FirstPersonDrawer(Constants.VIEW_WIDTH,Constants.VIEW_HEIGHT,
				Constants.MAP_UNIT,Constants.STEP_SIZE, mazecells, seencells, 10, mazedists.getDists(), mazew, mazeh, rootnode, this)) ;
		// order of registration matters, code executed in order of appearance!
		addView(new MapDrawer(gw, Constants.VIEW_WIDTH,Constants.VIEW_HEIGHT,Constants.MAP_UNIT,Constants.STEP_SIZE, mazecells, seencells, 10, mazedists.getDists(), mazew, mazeh, this)) ;
	}
		...
		case Constants.STATE_PLAY:
			...
			case 'k': case '8':
				...
				try {
					...
					if(driver.equals("Wizard")) {
						((Wizard) robotDriver).start();
					}
					if(driver.equals("Wall Follower"))
						((WallFollower) robotDriver).start();
					if(driver.equals("Curious Mouse"))
						((CuriousMouse) robotDriver).start();
				} catch (Exception e) {
