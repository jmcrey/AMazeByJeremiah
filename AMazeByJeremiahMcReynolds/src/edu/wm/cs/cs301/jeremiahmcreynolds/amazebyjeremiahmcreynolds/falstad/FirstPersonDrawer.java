...
public class FirstPersonDrawer extends DefaultViewer {
	 ...
	 */
	@Override
	public void redraw(Maze maze, int state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		...
		//maze.getGraphics().setGraphics(maze.maze.getGraphics().getCanvas());
		this.rset = rset ;
		...
		maze.getGraphics().setGraphicsColor("black");
		maze.getGraphics().fillRect(0, 0, view_width, view_height/2);
		// draw dark gray background on upper half
		maze.getGraphics().setGraphicsColor("darkGray");
		maze.getGraphics().fillRect(0, view_height/2, view_width, view_height/2);
		// set color to white and draw what ever can be seen from the current position
		maze.getGraphics().setGraphicsColor("white");
		rset.set(0, view_width-1); // reset set of ranges to set with single new element (0,width-1)
		// debug: reset counters
		//maze.getGraphics().redrawPerson(maze.gw, state, px, py, view_dx, view_dy, walk_step, view_offset, rset, ang);
	...
	private boolean boundingBoxIsVisible(int ymax, int ymin, int xmin, int xmax) {
		...
		maze.getGraphics().setPoint(x1, x2);
		return (rset.intersect(maze.getGraphics()));
	}
	/**
	...
	private void drawSegment(Seg seg, int ox1, int y1, int ox2, int y2) {
		...
		int x1i = x1;
		int xd = x2-x1;
		maze.getGraphics().setCol(seg.col.getR(), seg.col.getG(), seg.col.getB());
		//maze.getGraphics().setGraphicsColor(seg.getGraphics().getColor().toString());
//		maze.getGraphics().setGraphicsColor("green");
		boolean drawn = false;
		...
		while (x1i <= x2) {
			...
			maze.getGraphics().setPoint(x1i, x2);
			if (!rset.intersect(maze.getGraphics()))
				break;
			x1i = maze.getGraphics().getPoint().x;
			int x2i = maze.getGraphics().getPoint().y;
			...
			maze.getGraphics().fillPolygon(xps, yps, 4);
			drawn = true;
			rset.remove(x1i, x2i);
			x1i = x2i+1;
