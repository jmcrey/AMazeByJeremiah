...
import edu.wm.cs.cs301.jeremiahmcreynolds.amazebyjeremiahmcreynolds.ui.MyActivity;
...
public class MazeBuilder implements Runnable {
	...
	private BSPBuilder b;
	/**
	 * Constructor for a randomized maze generation
	 */
	public MazeBuilder(GraphicsWrapper gw){
	 ...
	 */
	public MazeBuilder(boolean deterministic, GraphicsWrapper gw){
...
//		buildThread.start();
	...
	public BSPNode initRootnode(GraphicsWrapper gw){
		b.setGraphics(gw);
		root = b.generateBSPNodes();
		return root;
	}
	...
	public void run() {
		...
		try {
			...
			b = new BSPBuilder(gw, maze, dists, cells, width, height, colchange, expectedPartiters) ;
			root = b.generateBSPNodes();
			}
			Thread.sleep(SLEEP_INTERVAL) ; // test if thread has been interrupted, i.e. notified to stop
