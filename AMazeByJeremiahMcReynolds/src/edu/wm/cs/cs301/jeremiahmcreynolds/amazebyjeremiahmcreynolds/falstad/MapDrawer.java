...
public class MapDrawer extends DefaultViewer {
	 ...
	 * @param mazeh TODO
	 */
	public MapDrawer(GraphicsWrapper gw, int width, int height, int map_unit, int step_size, Cells mazecells, Cells seencells, int map_scale, int[][] mazedists, int mazew, int mazeh, Maze maze){
		view_width = width ;
		view_height = height ;
	...
	@Override
	public void redraw(Maze maze, int state, int px, int py,
...
//		gw.setGraphics(this.maze.panel.getBufferGraphics());
		//gw.getGraphics().setColor(Color.white);
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				gw.setGraphicsColor(seencells.hasWallOnTop(x, y) ? "white" : "gray");
				if ((seencells.hasWallOnTop(x, y) || showMaze) && w)
					gw.drawLine2(nx1, ny1, nx2, ny1);
				...
				gw.setGraphicsColor(seencells.hasWallOnLeft(x, y) ? "white" : "gray");
//				if ((seencells.hasWallOnLeft(x, y) || showMaze) && w)
					gw.drawLine2(nx1, ny1, nx1, ny2);
			}
		if (showSolution) {
...
//		gw.setGraphics(maze.panel.getBufferGraphics());
		...
		gw.fillOval(ctrx-cirsiz/2, ctry-cirsiz/2, cirsiz, cirsiz);
		...
		gw.drawLine2(ctrx, ctry, aptx, apty);
		gw.drawLine2(aptx, apty, aptx2 + ptofx, apty2 + ptofy);
		gw.drawLine2(aptx, apty, aptx2 - ptofx, apty2 - ptofy);
	}
	...
	public void draw_solution(int offx, int offy, int px, int py) {
		...
		//gw.getGraphics().setColor(Color.yellow);
		...
		while (d > 1) {
			...
			// do the graphics
			gw.drawLine2(nx1, ny1, nx1+ndx, ny1+ndy);
